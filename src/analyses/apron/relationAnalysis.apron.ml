...
struct
  ...
  let combine ctx r fe f args fc fun_st (f_ask : Queries.ask) =
    ...
    let tainted = f_ask.f Queries.Tainted in
    let new_rel = RD.keep_filter st.rel (fun var ->
        ...
        | Some ((Local v | Global v)) when not (Queries.TaintS.mem v tainted) -> true (* keep locals and globals, which have not been touched by the call *)
