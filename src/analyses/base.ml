    ...
    Q.LS.fold (fun (v, o) st -> 
      if CPA.mem v fun_st.cpa then
        ...
        | Some (`Array _) when (get_string "ana.base.arrays.domain") = "partitioned" -> begin
          (* partitioned arrays cannot be copied by lvalues, so if tainted just copy the whole callee value for the array variable *)
        ...
        | _ -> begin
          ...
          (* This Line produces an error for regtest 06 14 line 43: Cilfacade.typeOfLval lval *) 
          ...
          match partDep with 
          | None -> st'
          (* if a var partitions an array, add all cpa-info for arrays it may partition from callee *)
